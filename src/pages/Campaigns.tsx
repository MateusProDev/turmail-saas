...
export default function Campaigns(){
  ...
  const [tenantOptions] = useState<Array<{id:string,role:string}>>([])
  ...
  const createCampaign = async () => {
    if (!subject || !htmlContent) { setResult('Subject and content are required'); return }
    setResult(null)
    try {
      const to = recipientsText.split(/[\n,;]+/).map(s => s.trim()).filter(Boolean).map(email => ({ email }))
      ...
      {showForm && (
        <div className="bg-white rounded shadow p-4 mb-6">
          ...
          <div className="grid gap-2 mt-3">
            ...
            <textarea value={htmlContent} onChange={e => setHtmlContent(e.target.value)} className="border rounded px-3 py-2 h-36" placeholder="Conteúdo HTML" />
            ...
            <div className="flex gap-2 items-center">
              ...
              <select value={selectedTenant ?? ''} onChange={e => { setSelectedTenant(e.target.value || null); if (e.target.value) refreshCampaignsByTenant(e.target.value) }} className="border rounded px-3 py-2">
